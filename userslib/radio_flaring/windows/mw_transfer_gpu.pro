pro MW_Transfer_GPU,parms,rowdata,path,parmin,datain,info=info

 if n_elements(path) eq 0 then begin
  dirpath=file_dirname((ROUTINE_INFO('MW_Transfer_GPU',/source)).path,/mark)+'binaries\'
  path=dirpath+(!version.memory_bits eq 64?'MW_Transfer_GPU_64.dll':'MW_Transfer_GPU_32.dll')
 end
 ...
 if arg_present(info) then begin
    ...
    Nvox=1L
    Npix=1L
    dummy_parmin=fltarr(n_elements(parms),Nvox,Npix)
    dummy_parmin[*]=parms.value
    dummy_datain=fltarr(7,parms[18].value,Npix)
   ...
   Nvox=sz[1]  
   Nparms=sz[2]
   if n_elements(datain) eq 0 then datain=fltarr(7,Nfreq,Npix)
   datain[*]=0 ;######
   if n_elements(parmin) eq 0 then parmin=fltarr(Nparms,Nvox,Npix)
