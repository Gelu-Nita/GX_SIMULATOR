pro GS_Transfer,parms,rowdata,path,parmin,datain,info=info

 if n_elements(path) eq 0 then begin
  dirpath=file_dirname((ROUTINE_INFO('GS_Transfer',/source)).path,/mark)+'binaries\'
  path=dirpath+(!version.memory_bits eq 64?'GS_Transfer_Slice_64.dll':'GS_Transfer_Slice_32.dll')
 end
