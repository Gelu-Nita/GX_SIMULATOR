pro GS_Transfer_DP,parms,rowdata,path,datain,info=info
 ...
 if n_elements(path) eq 0 then begin
  dirpath=file_dirname((ROUTINE_INFO('GS_Transfer_DP',/source)).path,/mark)+'binaries\'
  path=dirpath+(!version.memory_bits eq 64?'GS_Transfer_Slice_DP_64.dll':'GS_Transfer_Slice_DP_32.dll')
 end
