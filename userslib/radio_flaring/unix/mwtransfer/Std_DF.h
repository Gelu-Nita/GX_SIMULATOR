...
#define SIN 111 //sin^N distribution
#define EXP 2 //exponential (on pitch-angle cosine) distribution
...
class Std_DF : public DF
{
 ...
 void FE(double E, double mu, 
		 ...
		 double *g1, double *g2);
 Std_DF(double n0, double *ParmIn);
 ~Std_DF();
};
...
class Multi_Std_DF
{
 ...
 int OK;
 double n_b;
 Multi_Std_DF(double n0, double *ParmIn);
 ~Multi_Std_DF();
};