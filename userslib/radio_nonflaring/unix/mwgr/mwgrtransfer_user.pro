pro MWGRTransfer_user,parms,rowdata,path,parmin,datain,info=info
 
 if n_elements(path) eq 0 then begin
   dirpath=getenv('HOME')+'/mwtransfer/gr'
   path=dirpath+'/MWGRTransfer.so'
   if ~file_test(path) then begin
     file_mkdir, dirpath
     src_path=file_dirname((ROUTINE_INFO('mwgrtransfer_user',/source)).path)
