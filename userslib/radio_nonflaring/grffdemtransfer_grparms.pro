pro grffdemtransfer_grparms,parms,rowdata,nparms,rparms,path,parmin,datain,freqlist,logtdem=logtdem,$
...
if n_elements(path) eq 0 then path=gx_libpath('grffdem')
 if arg_present(info) then begin
   ...
   if n_elements(grparms) eq 0 then grparms=dblarr(5,Nfreq,nparms[7]-nparms[6]+1,Npix)
