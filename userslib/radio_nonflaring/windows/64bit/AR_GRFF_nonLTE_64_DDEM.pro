pro AR_GRFF_nonLTE_64_DDEM,parms,rowdata,path,parmin,datain,info=info
 if n_elements(path) eq 0 then begin
  dirpath=file_dirname((ROUTINE_INFO('AR_GRFF_nonLTE_64_DDEM',/source)).path,/mark)
  path=dirpath+'AR_GRFF_nonLTE_64_DDEM'
 end
 if arg_present(info) then begin
    if n_elements(parms) gt 0 then dummy=temporary(parms)
    if n_elements(info) eq 0 then begin
      result=call_external(path,'get_parms_',/F_VALUE,/unload )
    ...
    test_call=call_external(path,'get_mw_',Nvox,dummy_parmin,dummy_datain,/unload )
    info={parms:parms,$
   ...
   for pix=0, Npix-1 do begin
         ...
         RESULT=call_external(path,'get_mw_',Nvox,parmin,datain,/unload )
