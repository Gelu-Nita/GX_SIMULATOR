pro GRFF_DEM_Transfer_Slice_64,parms,rowdata,nparms,rparms,path,parmin,datain,logtdem=logtdem,$
dem_run=dem_run,ddm_run=ddm_run,qrun=qrun,lrun=lrun,use_dem=use_dem,has_ddm=has_ddm,info=info
 if n_elements(path) eq 0 then begin
  dirpath=file_dirname((ROUTINE_INFO('GRFF_DEM_Transfer_Slice_64',/source)).path,/mark)
 ...
 if arg_present(info) then begin
    ...
    if n_elements(info) eq 0 then begin
      ...
      rparms=info.rparms
    endelse
    nfreq=nparms[gx_name2idx(nparms,'N_freq')].value
    ...
    tdem=10d0^logtdem
    test_call=call_external(path, 'GET_MW_SLICE', ndat, rdat,dummy_parmin, tdem, dem_arr, ddm_arr, dummy_datain,/unload)
