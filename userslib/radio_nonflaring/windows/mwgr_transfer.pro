pro MWGR_Transfer,parms,rowdata,path,parmin,datain,info=info

 if n_elements(path) eq 0 then begin
  dirpath=file_dirname((ROUTINE_INFO('MWGR_Transfer',/source)).path,/mark)+'binaries\'
  path=dirpath+(!version.memory_bits eq 64?'MWGR_Transfer_64':'MWGR_Transfer_32')
 end
