...
dem_run=dem_run,ddm_run=ddm_run,qrun=qrun,lrun=lrun,use_dem=use_dem,has_ddm=has_ddm,info=info
 if n_elements(path) eq 0 then begin
  dirpath=file_dirname((ROUTINE_INFO('GRFF_DEM_Transfer',/source)).path,/mark)+'binaries\'
  path=dirpath+(!version.memory_bits eq 64?'GRFF_DEM_Transfer_64':'GRFF_DEM_Transfer_32')
 end
