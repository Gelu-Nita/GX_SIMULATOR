pro AR_GRFF_nonLTE,parms,rowdata,path,parmin,datain,info=info
 if n_elements(path) eq 0 then begin
  dirpath=file_dirname((ROUTINE_INFO('AR_GRFF_nonLTE',/source)).path,/mark)+'binaries\'
  path=dirpath+(!version.memory_bits eq 64?'AR_GRFF_nonLTE_64':'AR_GRFF_nonLTE_32')
 end
