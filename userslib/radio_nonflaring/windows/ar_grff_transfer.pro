pro AR_GRFF_Transfer,parms,rowdata,path,parmin,datain,info=info
 if n_elements(path) eq 0 then begin
  dirpath=file_dirname((ROUTINE_INFO('AR_GRFF_Transfer',/source)).path,/mark)+'binaries\'
  path=dirpath+(!version.memory_bits eq 64?'AR_GRFF_Transfer_64':'AR_GRFF_Transfer_32')
 end
