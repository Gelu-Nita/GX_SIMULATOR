 ...
 if arg_present(info) then begin
  ...
  return
   ...
   for pix=0, Npix-1 do begin
     ...
     trmask_idx=where((ulong(rowparms[idx.parms.voxid,*]) and gx_voxelid(/euv)) ne 0,trmask_count)
     if trmask_count gt 0 then rowdata[pix,*,4]=1
   ...
   if AddTR gt 0 then rowdata[*,*,0]=rowdata[*,*,1]+rowdata[*,*,2] else rowdata[*,*,0]=rowdata[*,*,1]
end