pro gx_addlines2box, box,tr_height_km, status=status, physLength=physLength, avField=avField, startIdx=startIdx, endIdx=endIdx,lib_path=lib_path,elapsed_time=elapsed_time,_extra = _extra
  ...
  status=reform(status,size(box.bx,/dim))
  startidx=reform(startidx,size(box.bx,/dim))
  endidx=reform(endidx,size(box.bx,/dim))
  avField=reform(avField,size(box.bx,/dim))
  physLength=reform(physLength,size(box.bx,/dim))
  gx_sortlines,status,startidx,endidx
  box=rep_tag_value(box,status,'status',/no_copy,/duplicate)
  box=rep_tag_value(box,startidx,'startidx',/no_copy,/duplicate)
  box=rep_tag_value(box,endidx,'endidx',/no_copy,/duplicate)
  box=rep_tag_value(box,avField,'avField',/no_copy,/duplicate)
  box=rep_tag_value(box,physLength,'physLength',/no_copy,/duplicate)
;  idx=where((status and 4L) eq 4L)
;  oidx=where((status and 16L) eq 16L)
;  bmed=avField[idx]
;  obmed=avField[oidx]
